...
static void SetSysClock(void) {
  ...
  if (HSEStatus == (uint32_t) 0x01) {
   ...
   configuration. User can add here some code to deal with this error */
    /* For boards without HSE, this is normal operation.
     * Set HSI as clock input, use PLL to generate the 48MHz clock.
     */

    /* Clear the HSE enable - we don't need it anymore */
    RCC->CR &= ~((uint32_t) RCC_CR_HSEON);
